import numpy as np
import cv2
from core_utils.bounding_box import resizeBoundingBox
from core_utils.predictor_keras import predict
...
def detect_faces_with_mask(img, face_model, mask_model, classes=['no_mask', 'mask'], confidence=0.5, bbox_percentage=0.1, predictor='keras', target_size=(160,160)):
    ...
    for face in faces:
        ...
        if cnf >= confidence:
            ...
            cv2.rectangle(img_color, (x, y), (x + w, y + h), color, fontThickness)
            cv2.putText(img_color, pred.replace("_", " ").capitalize(), (x-2, y-7), fontFace, fontScale, color, fontThickness)
    return img_color
